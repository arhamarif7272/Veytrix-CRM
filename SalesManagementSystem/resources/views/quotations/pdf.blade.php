...
<head>
    ...
    <style>
        ...
        .totals-table { width: 40%; margin-left: auto; margin-bottom: 30px; border-collapse: collapse; }
        .totals-table td { padding: 6px 10px; }
        .total-row { border-top: 2px solid #1e293b; font-weight: bold; font-size: 15px; color: #4f46e5; }
...
<body>
        ...
        <table>
            <tr>
                <td>
                    ...
                    <div style="color: #64748b; font-size: 12px;">support@veytrix.com &bull; +1 (800) 555-0199</div>
                </td>
                <td>
                    <div class="doc-title">QUOTATION</div>
                    <div class="doc-meta" style="font-weight: bold; color: #4f46e5; font-size: 14px;">#{{ $quotation->number }}</div>
                    <div class="doc-meta">Date: {{ $quotation->created_at ? $quotation->created_at->format('M d, Y') : '' }}</div>
                    <div class="doc-meta">Valid Until: {{ $quotation->valid_until ? $quotation->valid_until->format('M d, Y') : '' }}</div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="client-info">
        <tr>
            <td style="width: 60%; vertical-align: top;">
                <div style="font-weight: bold; color: #64748b; font-size: 11px; text-transform: uppercase;">Quotation For:</div>
            ...
            <td style="width: 40%; vertical-align: top; text-align: right;">
                <div style="font-weight: bold; color: #64748b; font-size: 11px; text-transform: uppercase;">Status:</div>
                <div style="font-size: 14px; font-weight: bold; text-transform: uppercase; color: #4f46e5;">{{ $quotation->status }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="totals-table">
        ...
        <tr>
            ...
            <td class="text-right" style="color: #dc2626;">-${{ number_format($quotation->discount_amount, 2) }}</td>
        </tr>
        @endif
        <tr>
            <td style="color: #64748b;">Tax ({{ $quotation->tax_rate }}%):</td>
    ...
    <div class="notes-section">
        @if($quotation->notes)
            ...
            <div style="margin-bottom: 10px;">{{ $quotation->notes }}</div>
        @endif
        @if($quotation->terms)
